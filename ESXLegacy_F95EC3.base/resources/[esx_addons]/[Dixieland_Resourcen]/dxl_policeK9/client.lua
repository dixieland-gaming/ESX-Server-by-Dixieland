...
RegisterNetEvent('dxl_k9scipt:Notify', function(message, type)
	if Config.UseCustomNotify then
        TriggerEvent('dxl_k9scipt:CustomNotify',message, type)
			...
			exports.ox_target:addGlobalVehicle({
				{
					...
					event = 'dxl_k9script:searchingcar',
			...
			exports.ox_target:addGlobalPlayer({
				{ 
					...
					onSelect = function(entity)
						StopAttack()
						TriggerEvent('dxl_k9script:searching', tonumber(GetPlayerServerId(NetworkGetPlayerIndexFromPed(entity.entity))), entity.entity)
					end,
				...
				options = {
					{
						...
						action = function(entity)
							TriggerEvent('dxl_k9script:jobchecker')
						end,
					},
				},
				distance = 1.5 
			...
			exports[Config.ThirdEyeName]:AddGlobalPlayer({
				options = {
				  { 
					...
					action = function(entity)
						StopAttack()
						TriggerEvent('dxl_k9script:searching', tonumber(GetPlayerServerId(NetworkGetPlayerIndexFromPed(entity))), entity)
					end,
					canInteract = function(entity, distance, data)
					  if DoesEntityExist(Dog) then return true end
					  return false
...
RegisterNetEvent('dxl_k9script:jobchecker', function()
	if Config.JobRestriction or Config.ItemRestriction then
		...
		if Config.ItemRestriction and Config.JobRestriction then
			if hasItem and hasJob and hasRank then
				...
				TriggerEvent('dxl_k9scipt:Notify', Config.Lang['miss_reqs'], Config.LangType['error'])
			end
		elseif Config.ItemRestriction then
			if hasItem then
				allow = true
			else
				TriggerEvent('dxl_k9scipt:Notify', Config.Lang['no_item'], Config.LangType['error'])
			end
		elseif Config.JobRestriction then
			if hasJob and hasRank then
				allow = true
			elseif hasJob and not hasRank then
				TriggerEvent('dxl_k9scipt:Notify', Config.Lang['low_rank'], Config.LangType['error'])
			else
				TriggerEvent('dxl_k9scipt:Notify', Config.Lang['no_cop'], Config.LangType['error'])
			end
		end
		if allow then
			TriggerEvent("dxl_k9script:dogspawn")
		end
	else
		TriggerEvent('dxl_k9script:dogspawn')
	end
...
RegisterNetEvent('dxl_k9script:dogspawn', function()
	...
	if not DoesEntityExist(Dog) then
		...
		TriggerEvent('dxl_k9script:dogactions')
...
RegisterNetEvent('dxl_k9script:searchingcar', function()
	if DoesEntityExist(Dog) then
		...
		if Config.UseESX then
			...
			ESX.TriggerServerCallback('dxl_k9script:server:searchcar:ESX', function(cb)
				hasItem = cb
			end,data.plate)
		elseif Config.UseQBCore then
			VehicleData = QBCore.Functions.GetClosestVehicle()
			local plate = QBCore.Functions.GetPlate(VehicleData)
			QBCore.Functions.TriggerCallback('dxl_k9script:server:searchcar:QBCore', function(cb)
				hasItem = cb
			end,plate)
		end
		...
		TriggerEvent('dxl_k9scipt:Notify', Config.Lang['no_dog'], Config.LangType['error'])
	end
end)

RegisterNetEvent('dxl_k9script:dogactions', function()
	CreateThread(function()
        ...
        while DoesEntityExist(Dog) do
            ...
            if IsEntityAPed(targetentity) then
                ...
                if IsControlJustReleased(0,Config.SearchCommand) then
                    StopAttack()
                    TriggerEvent('dxl_k9script:searching', target,targetentity)
                elseif IsControlJustReleased(0,Config.AttackCommand) then
                    StopAttack()
                    TriggerEvent('dxl_k9scipt:client:attacking', targetentity)
                end
            end
            Wait(Sleep)
        end
...
RegisterNetEvent('dxl_k9scipt:client:attacking', function(target)
	Follow = false
	TaskCombatPed(Dog, target, 0, 16)
end)

RegisterNetEvent('dxl_k9script:searching', function(target,entity)
    local hasItem = false
    local DogCoord = GetEntityCoords(Dog)
    local PlayerCoord = GetEntityCoords(entity)
    if Config.UseESX then
		ESX.TriggerServerCallback('dxl_k9script:server:search:ESX', function(cb)
			hasItem = cb
		end,target) 
    elseif Config.UseQBCore then
		QBCore.Functions.TriggerCallback('dxl_k9script:server:search:QBCore', function(cb)
			hasItem = cb
		end,target)
    end
