...
lib.callback('dxl_doorlock:getDoors', false, function(data)
	doors = data
...
RegisterNetEvent('dxl_doorlock:setState', function(id, state, _, data)
	...
	if door.state == state and door.distance and door.distance < 20 then
		if Config.NativeAudio then
			RequestScriptAudioBank('dlc_dxldoorlock/dxldoorlock', false)
			local sound = state == 0 and door.unlockSound or door.lockSound or 'door_bolt'
			local soundId = GetSoundId()

			PlaySoundFromCoord(soundId, sound, door.coords.x, door.coords.y, door.coords.z, 'DLC_dxlDOORLOCK_SET', false, 0, false)
			ReleaseSoundId(soundId)
			ReleaseNamedScriptAudioBank('dlc_dxldoorlock/dxldoorlock')
		else
...
lib.callback.register('dxl_doorlock:inputPassCode', function()
...
local function useClosestDoor()
	...
	if gameTimer - lastTriggered > 500 then
		...
		TriggerServerEvent('dxl_doorlock:setState', ClosestDoor.id, ClosestDoor.state == 1 and 0 or 1)
	end
end
