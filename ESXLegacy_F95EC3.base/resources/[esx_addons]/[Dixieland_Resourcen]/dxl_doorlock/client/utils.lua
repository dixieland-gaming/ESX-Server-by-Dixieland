...
local function pickLock(entity)
	...
	if success then
		TriggerServerEvent('dxl_doorlock:setState', door.id, door.state == 1 and 0 or 1, true)
	end

	if rand == 1 then
		TriggerServerEvent('dxl_doorlock:breakLockpick')
...
RegisterNUICallback('createDoor', function(data, cb)
	...
	TriggerServerEvent('dxl_doorlock:editDoorlock', data.id or false, data)
	table.wipe(tempData)
end)

RegisterNUICallback('deleteDoor', function(id, cb)
	cb(1)
	TriggerServerEvent('dxl_doorlock:editDoorlock', id)
end)
...
local function openUi(id)
	...
	if not NuiHasLoaded then
		...
		SendNUIMessage({
			...
			data = lib.callback.await('dxl_doorlock:getSounds', false)
		})
	end
...
RegisterNetEvent('dxl_doorlock:triggeredCommand', function(closest)
