if not LoadResourceFile(lib.name, 'web/build/index.html') then
	error('Unable to load UI. Build dxl_doorlock or download the latest release.\n ^3https://github.com/bitpredator/dxl_doorlock/releases/latest/download/dxl_doorlock.zip^0')
end
...
lib.versionCheck('bitpredator/dxl_doorlock')
lib.locale()
...
exports('editDoor', function(id, data)
	...
	if door then
		...
		MySQL.update('UPDATE dxl_doorlock SET name = ?, data = ? WHERE id = ?', { door.name, encodeData(door), id })
		TriggerClientEvent('dxl_doorlock:editDoorlock', -1, id, door)
	end
end)

local soundDirectory = Config.NativeAudio and 'audio/dlc_dxldoorlock/dxldoorlock' or 'web/build/sounds'
...
lib.callback.register('dxl_doorlock:getSounds', function()
	return sounds
end)
...
local function createDoor(id, door, name)
	...
	if double then
		for i = 1, 2 do
			double[i].hash = joaat(('dxl_door_%s_%s'):format(id, i))
		...
		door.hash = joaat(('dxl_door_%s'):format(id))
	end
	...
	if type(door.items?[1]) == 'string' then
		...
		door.items = items
		MySQL.update('UPDATE dxl_doorlock SET data = ? WHERE id = ?', { encodeData(door), id })
	end
...
local function isAuthorised(playerId, door, lockpick)
	...
	if player then
		...
		if authorised ~= nil and door.passcode and not door.characters and not door.groups then
			authorised = door.passcode == lib.callback.await('dxl_doorlock:inputPassCode', playerId)
		end
	end
...
local sql = LoadResourceFile(cache.resource, 'sql/dxl_doorlock.sql')
...
MySQL.ready(function()
	...
	local response = MySQL.query.await('SELECT `id`, `name`, `data` FROM `dxl_doorlock`')
...
local function setDoorState(id, state, lockpick)
	...
	if door then
		...
		if authorised then
			door.state = state
			TriggerClientEvent('dxl_doorlock:setState', -1, id, state, source)
			...
			if door.autolock and state == 0 then
				SetTimeout(door.autolock * 1000, function()
					if door.state ~= 1 then
						...
						TriggerClientEvent('dxl_doorlock:setState', -1, id, door.state)
						TriggerEvent('dxl_doorlock:stateChanged', nil, door.id, door.state == 1)
					end
				end)
			end

			TriggerEvent('dxl_doorlock:stateChanged', source, door.id, state == 1, type(authorised) == 'string' and authorised)
...
RegisterNetEvent('dxl_doorlock:setState', setDoorState)
exports('setDoorState', setDoorState)

lib.callback.register('dxl_doorlock:getDoors', function()
	while not isLoaded do Wait(100) end

	return doors, sounds
end)

RegisterNetEvent('dxl_doorlock:editDoorlock', function(id, data)
	if IsPlayerAceAllowed(source, 'command.doorlock') then
		...
		if id then
			if data then
				MySQL.update('UPDATE dxl_doorlock SET name = ?, data = ? WHERE id = ?', { data.name, encodeData(data), id })
			else
				MySQL.update('DELETE FROM dxl_doorlock WHERE id = ?', { id })
			end

			doors[id] = data
			TriggerClientEvent('dxl_doorlock:editDoorlock', -1, id, data)
		else
			local insertId = MySQL.insert.await('INSERT INTO dxl_doorlock (name, data) VALUES (?, ?)', { data.name, encodeData(data) })
			local door = createDoor(insertId, data, data.name)

			TriggerClientEvent('dxl_doorlock:setState', -1, door.id, door.state, false, door)
		end
	end
end)

RegisterNetEvent('dxl_doorlock:breakLockpick', function()
...
lib.addCommand('doorlock', {
	...
	TriggerClientEvent('dxl_doorlock:triggeredCommand', source, args.closest)
end)