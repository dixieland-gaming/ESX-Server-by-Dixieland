...
MySQL.ready(function()
	...
	local query = 'INSERT INTO `dxl_doorlock` (`name`, `data`) SELECT ?, ? WHERE NOT EXISTS (SELECT 1 FROM `dxl_doorlock` WHERE `name` = ?)'
	local queries = {}

	for i = 1, fileCount do
		local fileName = files[i]
		local file = LoadResourceFile('dxl_doorlock', ('convert/%s.lua'):format(fileName))
		...
		if file then
			...
			if next(Config.DoorList) then
				...
				if MySQL.transaction.await(queries) then
					SaveResourceFile('dxl_doorlock', ('convert/%s.lua'):format(fileName), '-- This file has already been converted for dxl_doorlock and should be removed.\r\ndo return end\r\n\r\n' .. file, -1)
				end
