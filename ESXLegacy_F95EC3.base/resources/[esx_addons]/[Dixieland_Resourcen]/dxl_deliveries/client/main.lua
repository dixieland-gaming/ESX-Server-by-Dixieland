    ...
    if CurrentStatus ~= Status.DELIVERY_INACTIVE then
        ...
        if CurrentStatus == Status.PLAYER_REMOVED_GOODS_FROM_VEHICLE then
            ...
            if GetDistanceBetweenCoords(pCoords.x, pCoords.y, pCoords.z, DeliveryLocation.Item2.x, DeliveryLocation.Item2.y, DeliveryLocation.Item2.z, true) < 1.5 then
                TriggerServerEvent("dxl_deliveries:finishDelivery:server", CurrentType)
                PlaySound(-1, "Menu_Accept", "Phone_SoundSet_Default", false, 0, true)
...
function StartDelivery(deliveryType)
    TriggerServerEvent("dxl_deliveries:removeSafeMoney:server", deliveryType)
end
...
function FinishDelivery(deliveryType, safeReturn)
    ...
    TriggerServerEvent("dxl_deliveries:returnSafe:server", deliveryType, safeReturn)
...
CreateThread(function()
    ...
    TriggerServerEvent("dxl_deliveries:getPlayerJob:server")
end)
...
RegisterNetEvent("dxl_deliveries:setPlayerJob:client")
RegisterNetEvent("dxl_deliveries:startJob:client")

AddEventHandler("esx:setJob", function(job)
    PlayerJob = job.name
end)

AddEventHandler("dxl_deliveries:setPlayerJob:client", function(job)
    print("Player job: " .. job)
    PlayerJob = job
end)

AddEventHandler("dxl_deliveries:startJob:client", function(deliveryType)
