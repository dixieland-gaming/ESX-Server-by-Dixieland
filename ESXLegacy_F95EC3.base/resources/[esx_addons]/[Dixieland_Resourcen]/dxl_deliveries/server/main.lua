...
-- Register events
RegisterServerEvent("dxl_deliveries:returnSafe:server")
RegisterServerEvent("dxl_deliveries:finishDelivery:server")
RegisterServerEvent("dxl_deliveries:removeSafeMoney:server")
RegisterServerEvent("dxl_deliveries:getPlayerJob:server")

-- Return safe deposit event
AddEventHandler("dxl_deliveries:returnSafe:server", function(deliveryType, safeReturn)
...
AddEventHandler("dxl_deliveries:finishDelivery:server", function(deliveryType)
    local xPlayer = ESX.GetPlayerFromId(source)
...
AddEventHandler("dxl_deliveries:removeSafeMoney:server", function(deliveryType)
    local xPlayer = ESX.GetPlayerFromId(source)
    local SafeMoney = 4000
    for k, v in pairs(Config.Safe) do
        if k == deliveryType then
    ...
    if PlayerMoney >= SafeMoney then
        ...
        TriggerClientEvent("dxl_deliveries:startJob:client", source, deliveryType)
    else
...
AddEventHandler("dxl_deliveries:getPlayerJob:server", function()
    local xPlayer = ESX.GetPlayerFromId(source)
    print("Player request job: " .. source .. ", " .. xPlayer.job.name)
    TriggerClientEvent("dxl_deliveries:setPlayerJob:client", source, xPlayer.job.name)
end)
