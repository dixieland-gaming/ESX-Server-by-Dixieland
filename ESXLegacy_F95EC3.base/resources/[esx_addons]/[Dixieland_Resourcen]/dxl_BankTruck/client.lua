...
RegisterNetEvent('dxl_BankTruck:Notify', function(message, type)
	if Config.UseCustomNotify then
        TriggerEvent('dxl_BankTruck:CustomNotify',message, type)
...
CreateThread(function()
    ...
    if Config.UseThirdEye then
        local eye_options = {{
            event = "dxl_BankTruck:RobberyCheck",
            icon = 'fas fa-phone',
                    ...
                    if Dist <= 50 then
                        ...
                        if Dist <= 3 then
                            ...
                            if IsControlJustReleased(0, 38) then
                                TriggerEvent('dxl_BankTruck:RobberyCheck')
                            end
                        end
                    end
                end
    ...
    if Config.UsePed then
        ...
        while true do
            ...
            if Dist <= 50 and not PedSpawned then
                TriggerEvent('dxl_BankTruck:SpawnNPC',Config.StartPed,Config.StartModel)
                PedSpawned = true
...
RegisterNetEvent('dxl_BankTruck:SpawnNPC',function(coords,model)
...
RegisterNetEvent('dxl_BankTruck:RobberyCheck', function()
    if not Config.AllowLeoRob and isLawEnforcement then
        TriggerEvent('dxl_BankTruck:Notify', Config.Lang['nocop'], Config.LangType['info'])
        return
    end
    if onjob then
        TriggerEvent('dxl_BankTruck:Notify', Config.Lang['onjob'], Config.LangType['info'])
        return
    end
    if CooldownRemaining > 0 then
        TriggerEvent('dxl_BankTruck:Notify', Config.Lang['indCD'], Config.LangType['info'])
        return
    end
    local StartRobbery = true
    local hasItem = true
    TriggerEvent('dxl_BankTruck:Notify', Config.Lang['checking'], Config.LangType['info'])
    if Config.RequireMinimumLEO then
        StartRobbery = false
        if Config.UseESX then
            ESX.TriggerServerCallback('dxl_BankTruck:PoliceAvailable:ESX', function(cb)
                StartRobbery = cb
            end)                                    
        elseif Config.UseQBCore then
            QBCore.Functions.TriggerCallback('dxl_BankTruck:PoliceAvailable:QBCore', function(cb)
                StartRobbery = cb
            end)
        end
        Wait(1000)
    ...
    if not StartRobbery then
        TriggerEvent('dxl_BankTruck:Notify', Config.Lang['mincops'], Config.LangType['error'])
        return
    end
    if Config.RequireStartItem then
        hasItem = false
        if Config.UseESX then
            ESX.TriggerServerCallback('dxl_BankTruck:itemTaken:ESX', function(cb)
                hasItem = cb
            end)                                    
        elseif Config.UseQBCore then
            QBCore.Functions.TriggerCallback('dxl_BankTruck:itemTaken:QBCore', function(cb)
                hasItem = cb
            end)
        end
        Wait(1000)
    ...
    if hasItem then
        ...
        TriggerServerEvent('dxl_BankTruck:Server:Guards')
        TriggerEvent('dxl_BankTruck:Notify', Config.Lang['startHeist'], Config.LangType['info'])
    else
        TriggerEvent('dxl_BankTruck:Notify', Config.Lang['noitem'], Config.LangType['error'])
    end
end)

RegisterNetEvent('dxl_BankTruck:Client:HesitSyncStarterStart', function(finish, start)
    BlipAdder(start, finish)
end)

RegisterNetEvent('dxl_BankTruck:Client:HesitSyncCopsStart', function(finish, start)
    if isLawEnforcement then
        CreateThread(function()
            BlipAdder(start, finish)
        end)
        TriggerEvent('dxl_BankTruck:Notify', Config.Lang['cop_notify'], Config.LangType['info'])
    end
end)

RegisterNetEvent('dxl_BankTruck:Client:BeginRoute', function(finish, time, start, list)
    GaurdList[time] = {}
    ...
    TriggerEvent('dxl_BankTruck:PoliceAlert', GetEntityCoords(vehicle))
    TriggerServerEvent('dxl_BankTruck:Server:TruckSync', finish, time, start, id)
    onjob = false
...
RegisterNetEvent('dxl_BankTruck:Client:TruckSync', function(time, inital, final, netid)
        ...
        if Config.UseThirdEye then
            if Config.ThirdEyeName == 'ox_target' then
                local options = {
                    {
                        onSelect = function()
                            TriggerServerEvent('dxl_BankTruck:Server:ArmExplosion', obj, time)
                            LootAnim(obj)
                        end,
                        icon = 'fas fa-truck',
                    ...
                    {
                        onSelect = function()
                            TriggerEvent('dxl_BankTruck:Loot', obj, netid, time)
                        end,
    ...
    while true do 
        ...
        if #(Pos-truckPos) <= 350 and DoesEntityExist(obj) then
            active = true
            TriggerEvent('dxl_BankTruck:Notify', Config.Lang['truck_ping'], Config.LangType['info'])
            break
...
RegisterNetEvent('dxl_BankTruck:Client:ArmExplosion', function(obj, number)
    EPlant[number] = true
...
RegisterNetEvent('dxl_BankTruck:Loot', function(obj, netid, time)
    if not DoesEntityExist(obj) then return end
    TriggerServerEvent('dxl_BankTruck:Server:ThirdEyeSync', netid, time)
    LootAnim(obj)
    TriggerServerEvent('dxl_BankTruck:Server:HeistReward')
end)

RegisterNetEvent('dxl_BankTruck:Client:ThirdEyeSync', function(netid, time)
    ELoot[time] = true
    local obj = NetworkGetEntityFromNetworkId(netid)
    while not DoesEntityExist(obj) do
        if DoesEntityExist(obj) then
...
function TruckSpawner(time, start, finsh, truck)
    ...
    if limit <= 0 then TriggerEvent('dxl_BankTruck:Notify', src, Config.Lang['time_limit'], Config.LangType['error']) return false end
