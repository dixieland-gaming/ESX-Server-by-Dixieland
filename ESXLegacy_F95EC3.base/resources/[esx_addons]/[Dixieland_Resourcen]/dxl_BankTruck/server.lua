if Config.UseESX then
    ...
    ESX.RegisterServerCallback('dxl_BankTruck:PoliceAvailable:ESX',function(source,cb)
    ...
    ESX.RegisterServerCallback('dxl_BankTruck:itemTaken:ESX',function(source,cb)
...
elseif Config.UseQBCore then
    ...
    QBCore.Functions.CreateCallback('dxl_BankTruck:PoliceAvailable:QBCore', function(source, cb)
        local cops = 0
    ...
    QBCore.Functions.CreateCallback('dxl_BankTruck:itemTaken:QBCore', function(source, cb)
...
RegisterNetEvent('dxl_BankTruck:Server:HeistReward', function()
	local src = source
    local type = nil
	local Player = nil
    ...
    if math.random(1, 100) <= Config.RareLootChance then
        ...
        TriggerClientEvent('dxl_BankTruck:Notify', src, Config.Lang['gained_rare'], Config.LangType['success'])
    end
end)
--- Guard Spawner
RegisterNetEvent('dxl_BankTruck:Server:Guards', function()
    local List = {}
    local time = os.time()
    ...
    TriggerClientEvent("dxl_BankTruck:Client:HesitSyncStarterStart", source, nil, route)
    TriggerClientEvent("dxl_BankTruck:Client:HesitSyncCopsStart", -1, nil, route)
    TriggerClientEvent("dxl_BankTruck:Client:BeginRoute", source, nil, time, route, List)
end)

-- Syncs
RegisterNetEvent('dxl_BankTruck:Server:TruckSync', function(finish, time, start, id)
    TriggerClientEvent("dxl_BankTruck:Client:TruckSync", -1, time, start, finish, id)
end)

RegisterServerEvent('dxl_BankTruck:Server:ThirdEyeSync', function(name, time)
    TriggerClientEvent('dxl_BankTruck:Client:ThirdEyeSync', -1, name, time)
end)

RegisterServerEvent('dxl_BankTruck:Server:ArmExplosion', function(name, time)
    TriggerClientEvent('dxl_BankTruck:Client:ArmExplosion', -1, name, time)
end)
...
-- Exploit Trigger
RegisterServerEvent('dxl_BankTruck:ThatIsAThing', function(server)
    if server ~= nil then
        DropPlayer(server, "Go hack somewhere else.")
    end
    Print("\n\n\n\nWARNING WARNING WARNING\nPlayer ID "..tostring(server).." was kicked for attempting to exploit dxl_BankTruck. It is recommended you ban them.\nnWARNING WARNING WARNING\n\n\n\n")
end)
