...
--[[    if type.skillCheck then 
    ...
    if progress({
        ...
        local pickedFruit = lib.callback.await('dxl_fruitpicking:pickFruit', false, type)
        if pickedFruit then 
...
local function openBuyerMenu(buyer)
    ...
    for item, itemInfo in pairsInOrder(buyer.items) do
        buyerOptions[#buyerOptions + 1] = {
            ...
            arrow = true,
            onSelect = function()
                lib.showContext('dxl_fruitpicking_buyer_'..item)
            end
        }
        lib.registerContext({
            id = 'dxl_fruitpicking_buyer_'..item,
            menu = 'dxl_fruitpicking_buyer',
            onExit = function()
                buyerOpen = false
            ...
            options = {
                {
                    ...
                    arrow = true,
                    onSelect = function()
                        local sellFruit = lib.callback.await('dxl_fruitpicking:sellFruit', false, true, item, buyer)
                        if sellFruit then 
                ...
                },
                {
                    ...
                    arrow = true,
                    onSelect = function()
                        local sellFruit = lib.callback.await('dxl_fruitpicking:sellFruit', false, false, item, buyer)
                        if sellFruit then 
                            stevo_lib.Notify((locale('notify.sold'):format(itemInfo.label, sellFruit)), 'success', 3000)
                        else 
                            stevo_lib.Notify((locale('notify.notenough')):format(itemInfo.label), 'error', 3000)
    ...
    lib.showContext('dxl_fruitpicking_buyer')
