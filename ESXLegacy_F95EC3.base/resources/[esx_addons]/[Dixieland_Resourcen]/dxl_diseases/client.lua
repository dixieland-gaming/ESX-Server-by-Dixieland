...
RegisterNetEvent('dxl_MedicalDiseases:Notify', function(message, type)
	if Config.UseCustomNotify then
        TriggerEvent('dxl_MedicalDiseases:CustomNotify',message, type)
...
RegisterNetEvent('dxl_MedicalDiseases:CureIllness', function(type, usage)
    if type == 'allowed' then
        if isSick then
            if usage == sickType or usage == 'all' then
                ResetSickness()
                TriggerEvent('dxl_MedicalDiseases:Notify', Config.Lang['cured'], Config.LangType['success'])
            else
                TriggerEvent('dxl_MedicalDiseases:Notify', Config.Lang['wrong_medicine'], Config.LangType['error'])
            end
        else
            TriggerEvent('dxl_MedicalDiseases:Notify', Config.Lang['not_sick'], Config.LangType['error'])
        end
    end
end)
...
function SickThread()
    ...
    while true do
        ...
        if getSick <= Config.IllnessChance and not isSick then
            TriggerEvent('dxl_MedicalDiseases:Notify', Config.Lang['feel_sick'], Config.LangType['info'])
            isSick = true
        ...
        if CommandUse() then
            if arg[1] then
                if #(GetEntityCoords(PlayerPedId())-GetEntityCoords(GetPlayerPed(GetPlayerFromServerId(tonumber(arg[1]))))) <= Config.CommandDistance then
                    TriggerServerEvent('dxl_MedicalDiseases:CommandHeal', tonumber(arg[1]))
                 else
                    TriggerEvent('dxl_MedicalDiseases:Notify', Config.Lang['too_far'], Config.LangType['error'])
                end
            else
                TriggerEvent('dxl_MedicalDiseases:Notify', Config.Lang['no_id'], Config.LangType['info'])
            end
        else
            TriggerEvent('dxl_MedicalDiseases:Notify', Config.Lang['wrong_job'], Config.LangType['error'])
        end
    end, false)
