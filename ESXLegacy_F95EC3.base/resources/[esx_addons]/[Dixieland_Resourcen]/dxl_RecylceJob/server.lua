...
RegisterNetEvent('dxl_RecylceJob:Server:ActivityUpdater', function(active)
    if active then
        ...
        TriggerClientEvent('dxl_RecylceJob:Client:ActivityUpdater', -1, ActiveColour, Containers)
    else
...
RegisterNetEvent('dxl_RecylceJob:Payment', function(number, loc)
    local src = source
    local dist = #(loc-vector3(Trash.TrashSpot.x, Trash.TrashSpot.y, Trash.TrashSpot.z))
    if dist > Config.FurtherstBin then print('RecylceJob payment') TriggerEvent('dxl_RecylceJob:ThatIsAThing', "triggering payment more than 50 units away") return end
    ...
    TriggerClientEvent('dxl_RecylceJob:Notify',src, Config.Lang['offduty']..amount, Config.LangType['success'])
end)

RegisterNetEvent('dxl_RecylceJob:RandomItem', function(colour, loc)
    local src = source
    local dist = #(loc-vector3(Trash.TrashSpot.x, Trash.TrashSpot.y, Trash.TrashSpot.z))
    if dist > Config.FurtherstBin then print('RecylceJob item') TriggerEvent('dxl_RecylceJob:ThatIsAThing', "triggering random item selection more than 50 units away") return end
    if math.random(0,100) <= Config.GetRandomItemChance then
        ...
        TriggerClientEvent('dxl_RecylceJob:Notify',src, Config.Lang['item_find_1']..item.item..Config.Lang['item_find_2'], Config.LangType['success'])
    end 
end)

RegisterNetEvent('dxl_RecylceJob:ThatIsAThing', function(reason)
    local src = source
    DropPlayer(src, "Go hack somewhere else.")
    print("\n\n\n\nWARNING WARNING WARNING\nPlayer ID "..tostring(src).." was kicked for attempting to exploit dxl_RecylceJob for "..reason..". It is recommended you ban them.\nnWARNING WARNING WARNING\n\n\n\n")
end)
