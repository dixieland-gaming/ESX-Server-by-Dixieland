...
local OXoptions = {
    ...
    trash = { name = 'TrashSort', label = Config.Lang['grab_sort_item'], distance = 2, 
        onSelect = function()
            if not OnDuty then
                TriggerEvent('dxl_RecylceJob:Notify', Config.Lang['not_on_duty'], Config.LangType['info'])
            elseif CurrentSort then
                TriggerEvent('dxl_RecylceJob:Notify', Config.Lang['not_finished'], Config.LangType['error'])
            else
                BeginSorting()
            end
        end,
...
RegisterNetEvent('dxl_RecylceJob:Notify', function(message, type)
	if Config.UseCustomNotify then
        TriggerEvent('dxl_RecylceJob:CustomNotify',message, type)
    ...
    if Config.UseThirdEye and Config.ThirdEyeName ~= 'ox_target' then
        ...
        local op = {
            entry = {
                ...
                event = 'dxl_RecylceJob:Entry',
            },
            exit = {
                label = Config.Lang['request_exit'],
                event ='dxl_RecylceJob:Exit',
            },
            duty = {
                label = Config.Lang['sign_in'],
                event = 'dxl_RecylceJob:OnDuty',
            },
        }
                ...
                if dist <= 50 then 
                    ...
                    if dist <= 10 then
                        ...
                        if dist <= 3 and IsControlJustReleased(0, 38) then
                            TriggerEvent('dxl_RecylceJob:Entry')
                        end
                    end
                end
            end
...
RegisterNetEvent('dxl_RecylceJob:OnDuty', function()
    local dist = #(GetEntityCoords(PlayerPedId())-vector3(warehouselocation.x, warehouselocation.y, warehouselocation.z))
    if dist >= 50 then TriggerServerEvent('dxl_RecylceJob:ThatIsAThing', "triggering duty/payment more then 50 units away") return end
    if not OnDuty then
        OnDuty = true 
        TriggerEvent('dxl_RecylceJob:Notify', Config.Lang['onduty'], Config.LangType['success'])
    else
        OnDuty = false
        CurrentSort = false
        TriggerServerEvent('dxl_RecylceJob:Payment', itemSorted, GetEntityCoords(PlayerPedId()))
        itemSorted = 0
    end
end)

RegisterNetEvent('dxl_RecylceJob:Exit', function()
    itemSorted = 0
    CurrentSort = false
    if OnDuty then OnDuty = false TriggerEvent('dxl_RecylceJob:Notify', Config.Lang['early_leave'], Config.LangType['error']) end
    DoScreenFadeOut(100)
    while not IsScreenFadedOut() do Wait (10) end
    SetEntityCoords(PlayerPedId(), Config.EntryPoint.x, Config.EntryPoint.y, Config.EntryPoint.z)
    TriggerServerEvent('dxl_RecylceJob:Server:ActivityUpdater', false)
...
RegisterNetEvent('dxl_RecylceJob:Entry', function()
    DoScreenFadeOut(100)
    while not IsScreenFadedOut() do Wait (10) end
    SetEntityCoords(PlayerPedId(), warehouselocation.x+0.5, warehouselocation.y+0.5, warehouselocation.z)
    TriggerServerEvent('dxl_RecylceJob:Server:ActivityUpdater', true)
    SetEntityHeading(PlayerPedId(), warehouselocation.w)
    DoScreenFadeIn(1000)
    while not IsScreenFadedIn() do Wait(10) end
    TriggerEvent('dxl_RecylceJob:Notify', Config.Lang['inside_warehouse'], Config.LangType['info'])
    Wait(1000)
                ...
                if dist <= 25 then
                    ...
                    if dist <= 10 then
                        ...
                        if dist <= 2 then
                            ...
                            if IsControlJustReleased(0, 38) then
                                TriggerEvent('dxl_RecylceJob:Exit')
                            end
                        end
                    end
                end
                ...
                if dist <= 25 then
                    ...
                    if dist <= 10 then
                        ...
                        if dist <= 2 then
                            ...
                            if IsControlJustReleased(0, 38) then
                                TriggerEvent('dxl_RecylceJob:OnDuty')
                            end
                        end
                    end
                end
...
RegisterNetEvent('dxl_RecylceJob:Client:ActivityUpdater', function(colours, cons)
    Containers = cons
    ActiveColour = colours
end)

function SortItem(bincolour)
    if not OnDuty then TriggerEvent('dxl_RecylceJob:Notify', Config.Lang['not_on_duty'], Config.LangType['error']) return end
    if not CurrentSort then TriggerEvent('dxl_RecylceJob:Notify', Config.Lang['need_trash'], Config.LangType['error']) return end
    if CurrentSort ~= bincolour then TriggerEvent('dxl_RecylceJob:Notify', Config.Lang['wrong_bin']..Config.Lang[CurrentSort], Config.LangType['error']) return end
    ...
    SortAnimation()
    TriggerEvent('dxl_RecylceJob:Notify', Config.Lang['item_sorted'], Config.LangType['success'])
    TriggerServerEvent('dxl_RecylceJob:RandomItem', CurrentSort, GetEntityCoords(PlayerPedId()))
...
function BeginSorting()
    ...
    TriggerEvent('dxl_RecylceJob:Notify', Config.Lang['sort_item_1']..Config.Lang[info]..Config.Lang['sort_item_2'], Config.LangType['info'])
end
