...
function ManageJob()
    ESX.UI.Menu.CloseAll()
    ESX.TriggerServerCallback('dxl_gardening:callback:checkJobStatistics', function(experience)
        ...
        ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'gardening_manage_menu', {
            ...
            elseif data.current.name == 'redeem_points' then 
                if activeJob and not hasMowingVehicle then 
                    menu.close()
                    TriggerServerEvent('dxl_gardening:server:redeemPoints', collectedPoints)
                    RemoveBlip(PickUpBlip)
...
AddEventHandler('dxl_gardening:client:hasEnteredMarker', function(zone)
...
AddEventHandler('dxl_gardening:client:hasExitedMarker', function(zone)
    ESX.UI.Menu.CloseAll()
    CurrentAction = nil
end)
...
CreateThread(function()
    while true do
        ...
        if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
            ...
            TriggerEvent('dxl_gardening:client:hasEnteredMarker', currentZone)
        end

        if not isInMarker and HasAlreadyEnteredMarker then
            HasAlreadyEnteredMarker = false
            TriggerEvent('dxl_gardening:client:hasExitedMarker', LastZone)
        end
        Wait(sleep)
    end
end)
