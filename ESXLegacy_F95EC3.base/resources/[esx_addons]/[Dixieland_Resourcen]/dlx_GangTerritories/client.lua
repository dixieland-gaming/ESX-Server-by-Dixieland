...
CreateThread(function()
    if Config.UseESX then
        ...
        ESX.TriggerServerCallback('dxl_gang:GangGrab:ESX', function(cb)
            GangLocations = cb
        end)
    ...
    elseif Config.UseQBCore then
        ...
        QBCore.Functions.TriggerCallback('dxl_gang:GangGrab:QBCore', function(cb)
            GangLocations = cb
        end)
    end
...
RegisterNetEvent('dxl_gang:Notify', function(message, type)
	if Config.UseCustomNotify then
        TriggerEvent('dxl_gang:CustomNotify',message, type)
...
CreateThread(function()
    ...
    while true do
        ...
        if Dist <= 50 and not PedSpawned then
            TriggerEvent('dxl_gang:SpawnGangManager')
            PedSpawned = true
...
RegisterNetEvent('dxl_gang:SpawnGangManager',function()
    ...
    if Config.UseThirdEye then
        exports[Config.ThirdEyeName]:AddEntityZone('GangManager', NPC, {
            ...
            useZ = true
                ...
                options = {
                    {
                        event = 'dxl_gang:LookUpCurrentGangs', 
                        icon = 'fas fa-magnifying-glass', 
                        label = Config.Lang['LookUpCurrentGangs'],
                    },   
                    {
                        event = 'dxl_gang:MakeNewGang', 
            ...
            if Dist <= 20 then
                ...
                if Dist <= 3 then
                    ...
                    if IsControlJustReleased(0, 38) then
                        TriggerEvent('dxl_gang:LookUpCurrentGangs')
                    elseif IsControlJustReleased(0,47) then
                        TriggerServerEvent('dxl_gang:MakeNewGang')
                    end
                end
            end
...
RegisterNetEvent('dxl_gang:LookUpCurrentGangs',function()
    GangCheck()
    TriggerEvent('dxl_gang:Notify', Config.Lang['gang_list'], Config.LangType['info'])
    Wait(500)
    local GangList = {}
    ...
    elseif Config.OXLib then
        lib.registerMenu({
            ...
            TriggerEvent('dxl_gang:GangOptions', args.gang)
        end)
        lib.showMenu('GangList_ox')
    end
end)

RegisterNetEvent('dxl_gang:GangOptions',function(gang)
    ...
    if Config.NHMenu then
        ...
        table.insert(GangList, {
            ...
            event = 'dxl_gang:JoinGang',
            args = gang
        })
        table.insert(GangList, {
            header = Config.Lang['leave'], 
            event = 'dxl_gang:LeaveGang',
            args = gang
        })
        table.insert(GangList, {
            header = Config.Lang['manage'], 
            event = 'dxl_gang:ManageGang',
            args = gang
        })
    elseif Config.QBMenu then
        table.insert(GangList, {
    ...
    elseif Config.OXLib then
        lib.registerMenu({
            ...
            if args.join then
                TriggerEvent('dxl_gang:JoinGang', args.gang)
            elseif args.manage then
                TriggerEvent('dxl_gang:ManageGang', args.gang)
            else
                TriggerEvent('dxl_gang:LeaveGang', args.gang)
            end
        end)
...
RegisterNetEvent('dxl_gang:ManageGang',function(gang)
    if not Config.UseAddOnGang then return end
    if gang ~= GangName then TriggerEvent('dxl_gang:Notify', Config.Lang['not_gang_manage'], Config.LangType['error']) return end
    if not boss then TriggerEvent('dxl_gang:Notify', Config.Lang['not_boss_manage'], Config.LangType['info']) return end
    local GangList = nil
    local GangMenu = {}
    if Config.UseESX then
        ESX.TriggerServerCallback('dxl_gang:GetGangMembers:ESX', function(cb)
            GangList = cb
        end, gang)
    elseif Config.UseQBCore then
        QBCore.Functions.TriggerCallback('dxl_gang:GetGangMembers:QBCore', function(cb)
            GangList = cb
        end, gang)
    end
    ...
    for _, GangDetails in pairs(GangList) do
        ...
        if Config.NHMenu then
            table.insert(GangMenu, {
                header = name,
                event = 'dxl_gang:client:removeGangMember',
                args = id
            })
        elseif Config.QBMenu then
            table.insert(GangMenu, {
    ...
    elseif Config.OXLib then
        lib.registerMenu({
            ...
            TriggerEvent('dxl_gang:client:removeGangMember', args.oId)
        end)
        lib.showMenu('gangremovemenu_ox')
    end
end)
RegisterNetEvent('dxl_gang:client:removeGangMember',function(id)
    if not Config.UseAddOnGang or not boss then return end
    TriggerServerEvent('dxl_gang:server:RemoveGangMember',id)
end)
RegisterNetEvent('dxl_gang:JoinGang',function(gang)
    if not Config.UseAddOnGang then return end
    TriggerServerEvent('dxl_gang:server:JoinNewGang',gang)
end)
RegisterNetEvent('dxl_gang:LeaveGang',function(gang)
    if not Config.UseAddOnGang then return end
    TriggerServerEvent('dxl_gang:server:LeaveGang', gang)
end)

RegisterNetEvent('dxl_gang:MakeNewGang',function()
    GangCheck()
    TriggerEvent('dxl_gang:Notify', Config.Lang['new_gang_intro'], Config.LangType['info'])
    Wait(500)
    local ganginfo = {}
    if not Config.UseAddOnGang then
        local newgangname = string.upper(tostring(GangName))
        if NewGangNameCheck(newgangname) then
            TriggerServerEvent('dxl_gang:server:MakeNewGang', newgangname)
        else
            TriggerEvent('dxl_gang:Notify', Config.Lang['gang_exists'], Config.LangType['error'])
        end
    else
...
RegisterNetEvent('dxl_gang:IncreaseValue',function(gang)
    local ganginfo = {}
    if Config.NHInput then
        local keyboard, a = exports["nh-keyboard"]:Keyboard({
        ...
        if keyboard then
            ...
            if value > 0 then
                TriggerServerEvent('dxl_gang:server:IncreaseValue', value, gang)
            else
                TriggerEvent('dxl_gang:Notify', Config.Lang['zero_error'], Config.LangType['error'])
            end
        end
    elseif Config.QBInput then
        local info = exports['qb-input']:ShowInput({
...
RegisterNetEvent('dxl_gang:client:newGangOnline', function(data)
    TerritorySetUp(data)
end)

RegisterNetEvent('dxl_gang:PlaceMarker', function()
    local Player = PlayerPedId()
    ...
    TriggerServerEvent('dxl_gang:server:PlaceMarker', pos, GetEntityHeading(Player))
end)

RegisterNetEvent('dxl_gang:BurnAnim', function()
    local Player = PlayerPedId()
    FreezeEntityPosition(Player, true)
    RequestAnimDict("anim@amb@clubhouse@tutorial@bkr_tut_ig3@")
    while not HasAnimDictLoaded("anim@amb@clubhouse@tutorial@bkr_tut_ig3@") do
...
RegisterNetEvent('dxl_gang:BurnObj', function(sObj)
...
function GangCheck()
    if Config.UseESX then
        ESX.TriggerServerCallback('dxl_gang:GangCheck:ESX', function(cb, name, level)
    ...
    elseif Config.UseQBCore then
        QBCore.Functions.TriggerCallback('dxl_gang:GangCheck:QBCore', function(cb, name, level)
            for Line, Data in pairs(cb) do 
                IntGangs[Data.gang] = true
            end
            if Config.UseAddOnGang and name then
                ...
                if dist <= data.size then
                    ...
                    if not notify then
                        ...
                        if not gangMember then
                            TriggerEvent('dxl_gang:Notify', Config.Lang['enemy_territory'], Config.LangType['info'])
                        else
                            TriggerEvent('dxl_gang:Notify', Config.Lang['friendly_territory'], Config.LangType['info'])
                        end
                    end
                    if gangMember then
                        inZone = true
    ...
    if Config.UseThirdEye then
        if Config.ThirdEyeName == 'ox_target' then
            local eye_options = {
                {
                    onSelect = function()
                        TriggerEvent('dxl_gang:IncreaseValue', gang)
                    end,
                    ...
                    onSelect = function()
                        TriggerServerEvent('dxl_gang:server:BuyOut', gang, sObj)
                    end,
            ...
            if Dist <= 3 then
                ...
                if IsControlJustReleased(0, 38) then
                    if string.upper(tostring(GangName)) ~= gang then
                        TriggerEvent('dxl_gang:Notify', Config.Lang['3d_gang_interact_e'], Config.LangType['error'])
                    else
                        TriggerEvent('dxl_gang:IncreaseValue', gang)
                    end
                elseif IsControlJustReleased(0,47) then
                    if string.upper(tostring(GangName)) == gang then
                        TriggerEvent('dxl_gang:Notify', Config.Lang['3d_gang_interact_g'], Config.LangType['error'])
                    else
                        TriggerServerEvent('dxl_gang:server:BuyOut', gang, sObj)
                    end
                end
